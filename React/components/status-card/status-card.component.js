...
export default function StatusCard({ title1, title2, value1, value2, plan, containerStyle }) {
    ...
    return (
        <>
            <View style={[containerStyle, { paddingBottom: 17 }]}>
                <ImageBackground source={{ uri: plan }} style={{ backgroundColor: themeColors.darkPrimary, padding: 20, borderRadius: 35, overflow: 'hidden' }} imageStyle={{ opacity: 0.7, top: 10, left: 130, width: 245, height: 245 }}>
                    <Text style={styles.title}>{title1}</Text>
                    <Text style={styles.price}>{value1}</Text>
